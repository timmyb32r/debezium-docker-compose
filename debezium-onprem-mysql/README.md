# debezium 'on premise mysql' -> 'on premise kafka' CDC
...
4) 
    ```sh
    mysql -u $MYSQL_USER -p$MYSQL_PASS -h $MYSQL_HOSTNAME db1
...
5) check if topic created
    ...
    kcat -L \
         ...
         -t dbserver1.db1.timmyb32r_favourite_table \
         -X security.protocol=SASL_SSL \
         -X sasl.mechanisms=SCRAM-SHA-512 \
         -X sasl.username=$KAFKA_USER \
         -X sasl.password=$KAFKA_PASS \
         -X ssl.ca.location=./cert/CA.pem
      topic "dbserver1.db1.timmyb32r_favourite_table" with 1 partitions:
